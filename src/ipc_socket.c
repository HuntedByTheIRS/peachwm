 ...
 *
 * Function implementations for the IPC socket.
 */
...
ipc_send_raw(int fd, uint32_t type, const char *payload, size_t len)
{
	...
	while (sent < total) {
		...
		/* Advance iovecs past what was sent */
		size_t remain = sent;
		while (remain > 0 && msg.msg_iovlen > 0) {
			if (remain < msg.msg_iov[0].iov_len) {
				msg.msg_iov[0].iov_base =
					(void *)((uintptr_t)msg.msg_iov[0].iov_base
						 + remain);
				msg.msg_iov[0].iov_len -= remain;
				remain = 0;
			} else {
				remain -= msg.msg_iov[0].iov_len;
				msg.msg_iov++;
				msg.msg_iovlen--;
			}
		}
...
ipc_socket_init(void)
{
	...
	ipc_event_loop = wl_display_get_event_loop(dpy);
